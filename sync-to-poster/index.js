...
exports.syncPosterData = onCall({
  // Configuración regional y de memoria
  region: "us-central1",
  memory: "1GiB",
}, async (request) => {
  // Verificar autenticación
  if (!request.auth) {
    throw new Error("Se requiere autenticación para esta función");
  }
  ...
  if (!targetRestaurantId) {
    try {
      ...
      if (!tokensSnapshot.empty) {
        ...
        throw new Error('No se encontró ningún restaurante con token de Poster');
      }
    } catch (error) {
  ...
  try {
    ...
    if (!tokenDoc.exists) {
      throw new Error('No se encontró token de acceso para este restaurante');
    }
    ...
    console.error(`Error en sincronización para restaurante ${targetRestaurantId}:`, error);

    // Registrar error
    await admin.firestore().collection('fudi_logs').add({
      action: 'sync_poster_error',
      userId: request.auth.uid,
      restaurantId: targetRestaurantId,
