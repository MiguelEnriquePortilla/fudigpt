...
const OnboardingPage = () => {
  ...
  const handleConnectPoster = async () => {
    ...
    try {
      // Iniciar flujo de OAuth con Poster
      initiateOAuthFlow();
      
      // La redirección se maneja automáticamente, por lo que el resto del código
      // se ejecutará cuando el usuario regrese del flujo de autorización
    } catch (error) {
  ...
  // Maneja el callback de OAuth cuando el usuario regresa de la página de autorización
  // Esta función sería llamada desde una función de utilidad que captura el código de autorización
  const processAuthCode = async (code) => {
    try {
      // Procesar el código de autorización
      const result = await handleAuthCallback(code, currentUser.uid);
      ...
      if (result.success) {
        ...
        // Simular progreso de sincronización mientras se realiza la sincronización real
        ...
        // Obtener tokens y sincronizar datos
        const tokens = await getTokens(currentUser.uid);
        const syncResult = await syncAllData(currentUser.uid, tokens.accessToken);
        ...
        if (syncResult.success) {
          ...
          throw new Error(syncResult.message || 'Error al sincronizar datos');
        }
      } else {
        throw new Error(result.message || 'Error al procesar autorización');
      }
    } catch (error) {
